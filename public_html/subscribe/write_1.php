...
if (isset($_GET['validate']) and isset($_GET['id'])) {
  ...
  fclose($fp);
	if (!empty($em)) $mess='<b><font style="font-size:14px;">Спасибо за подтверждение расcылки! Адрес <a href="mailto:'.$em.'">'.$em.'</a> подтвержден.</b></font><br><font style="font-size:11px;">(Если ваш браузер не поддерживает автоматическую переадресацию, нажмите <a href=javascript:history.back(-1) style=text-decoration:none;>здесь</a>)';
  ...
  $title=2;
    ...
    $title=3;
        ...
        if(!eregi("^[^\n]+$",$name)) {
          $checkname="false";
          $mess='<b><font color=red>ОШИБКА:</font> Напишите, пожалуйста, корректное имя для получения рассылки!</b><br><br><a href="'.$mainurl.'" onclick="window.close();return false;">[закрыть]</a>';$title=0;
        }
        if(!isset($digits) or $digits==1) {
        $code = (eregi("^[0-9]+$",$_POST['getcode']))? $_POST['getcode']:"++";
        $code_confirm = trim($_POST['code_confirm']);
        ...
        if($code_confirm == $code) {
          ...
          $checkcode="false";
          $mess='<b><font color=red>ОШИБКА:</font> Напишите, пожалуйста, правильный цифровой код!</b><br><br><a href="'.$mainurl.'" onclick="window.close();return false;">[закрыть]</a>';$title=0;
        }
        } else $checkcode="true";
 //       if(!strstr($name,",") and !strstr($email,",")) {$checksign="true";} else {$mess='<b><font color=red>ОШИБКА:</font> Имя и адрес не должны содержать символ "," !</b><br><br><a href="'.$mainurl.'" onclick="window.close();return false;">[закрыть]</a>';$title=0;}
        ...
        if ($checkmail=="true" and $checkname=="true" and $checkcode=="true") {
...
<body>
<?php
echo '<div class="bl">'.$mess.'<br><br><font style="font-size:10px;font-family:verdana;">Powered by <a href="http://tsbs.ru" target="_blank">TSB Subscription</a> v. 1.38.4</font>';
$str = file("emails.txt");
echo '</body></html>';
