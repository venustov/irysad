...
function ItemCard($id,$name,$is_from,$urozhay,$status,$zakup,$rozn,$ed_izmer,$lot,$desc,$fold,$fold_img,$img){
...
$str = '<form action="basket.php" method="post" name="purchase"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
              <tr align="right">
                <td colspan="2" valign="bottom" style="padding-right:10px; padding-top:10px;"><span class="item_title"><a href="/shop/items/'.$id.'.html" title="подробнее..." id="'.$id.'">'.$name.'</a></span></td>
				<td>';
$x=0;
...
$list = $_COOKIE['list'];
	...
	if(!$x) $str .= '&nbsp;';
	elseif($x == 1) $str .= '<a href="'.$_SERVER['REQUEST_URI'].'#basket"><img src="images/shopping_cart.png" width="24" height="24" border="0"></a>';
	elseif($x == 2) $str .= '<a href="'.$_SERVER['REQUEST_URI'].'#list_yak"><img src="images/database.png" width="24" height="24" border="0"></a>';
$str .= '</td>
              </tr>
              <tr>
	...
	$str .= '<a href="photo.php?fold='.$fold_img.'&photo='.$img.'" target="_blank" alt="'.$name.'" title="Увеличить" onClick="popupWin = window.open(this.href, \'main_photo\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,left=339,top=140,width='.$size[0].',height='.$size[1].'\'); popupWin.focus(); return false;">';
	}
$size = getimagesize($small_photo);
$str .= '<img src="'.$small_photo.'" width="'.$size[0].'" height="'.$size[1].'" border="0">';
...
$str .= '</td>
                <td rowspan="6" align="right" valign="top"><img src="images/shtrih.jpg" width="24" height="80"></td>
              </tr>
              <tr>
                <td align="right" valign="bottom" style="padding-right:10px">';
	if($urozhay){
	$str .= '<a href="shop.php?urozhay='.$urozhay.'&prev=items" title="найти все продукты урожая этого года...">Урожай '.$urozhay.' года</a>';
	}
	else $str .= '&nbsp;';
$str .= '</td>
              </tr>
              <tr>
                <td align="right" valign="bottom" style="padding-right:10px"><a href="shop.php?status='.$status.'&prev=items" title="найти все такие продукты...">';
...
$str .= '</a></td>
	...
	if($rozn&&($rozn < $zakup)){
		if(!$_SESSION['logged_status']) $old_price = 1.5*$zakup;
		else $old_price = $zakup;
...
$str .= '</td>
	...
	if($rozn&&($rozn < $zakup)) $akt_price = $rozn;
	else $akt_price = $zakup;
	
	if(!$_SESSION['logged_status']) $akt_price *= 1.5;
$str .= $akt_price.' р';
	if($ed_izmer) $str .= '/'.$ed_izmer;
$str .= '</span></td>
              </tr>
	...
	if($rozn&&($rozn < $zakup)) $str .= 'УЦЕНКА!';
	else $str .= '&nbsp;';
$str .= '</td>
                <td align="right" valign="bottom" style="padding-right:10px; padding-bottom:10px">';
	if(!$x){
		if($status<=2) $str .= '<input type="submit" name="add_basket" value="добавить в корзину"><input type="hidden" name="id" value="'.$id.'"><input type="hidden" name="name" value="'.$name.'"><input type="hidden" name="ed_izmer" value="'.$ed_izmer.'"><input type="hidden" name="lot" value="'.$lot.'"><input type="hidden" name="price" value="'.$price.'">';
		else $str .= '<input type="submit" name="add_list" value="в лист ожидания"><input type="hidden" name="id" value="'.$id.'"><input type="hidden" name="name" value="'.$name.'"><input type="hidden" name="ed_izmer" value="'.$ed_izmer.'"><input type="hidden" name="lot" value="'.$lot.'"><input type="hidden" name="price" value="'.$price.'">';
	}
...
function ItemList($class,$zakup,$rozn,$ed_izmer,$id,$name,$is_from,$lot,$urozhay,$status){
	if($ed_izmer == 1) $ed_izmer = 'кг';
	elseif($ed_izmer == 2) $ed_izmer = '100 гр';
	elseif($ed_izmer == 3) $ed_izmer = 'уп.';
	elseif($ed_izmer == 4) $ed_izmer = 'шт.';
...
$str = '<tr class="'.$class.'"><td align="center">';
	if($status<=2) $str .= '<input type="checkbox" name="check_basket[]" value="'.$id.'|'.$name.'|'.$ed_izmer.'|'.$price.'">';
	else $str .= '&nbsp;';
$str .= '</td>
...
$str .= $akt_price.' р';
	if($ed_izmer) $str .= '/'.$ed_izmer;
$str .= '</div></td>
     <td align="left" style="padding-left:20px;"><a href="/shop/list/'.$id.'.html" title="подробнее..." id="'.$id.'">'.$name.'</a>';
...
$str .= '</td><td align="center">';
	if($status>2) $str .= '<input type="checkbox" name="check_list[]" value="'.$id.'|'.$name.'|'.$ed_izmer.'|'.$lot.'|'.$price.'">';
	else $str .= '&nbsp;';
$str .= '</td>
	 <td align="center">';
	if($rozn&&($rozn < $zakup)) $str .= '<div style="color:#FF0000;"><b>УЦЕНКА!</b></div>';
	else $str .= '&nbsp;';
$str .= '</td>
   </tr>';
return $str;
...
<form name="search" method="post" action=""><table bgcolor="#7D94C0">
    <tr>
      <td><img src="images/search.png" width="16" height="16" border="0"></td>
...
<?php
  ...
  <table width="100%" bgcolor="#7D94C0">';
		if($basket){
		echo '<tr>
      <td><img src="images/basket.png" width="16" height="16" border="0" id="basket"></td>
	...
	';
      ...
      <td width="100%" align="left" style="color:#000000;"><a href="'.$_SERVER['REQUEST_URI'].'#'.$value['id'].'">'.$value['name'].'</a></td>
		...
		if($list){
		echo '<tr>
      <td><img src="images/list.png" width="16" height="16" border="0" id="list_yak"></td>
	...
	';
		$items = unserialize($list);
			...
			$name = str_replace('"','&quot;',$value['name']);
			echo '<tr>
      <td><a href="basket.php?id='.$value['id'].'&del_from=list"><img src="images/remove_from_list.png" width="16" height="16" border="0" alt="Удалить из листа ожидания" title="Удалить из листа ожидания"></a></td>
      <td width="100%" align="left" style="color:#000000;"><a href="'.$_SERVER['REQUEST_URI'].'#'.$value['id'].'">'.$value['name'].'</a></td>
    </tr>';
			}
		}
...
Либо если есть розничная цена и она меньше закупочной, но только в том случае, когда товар есть в наличии*/
					...
					echo ItemCard($id,$name,$is_from,$urozhay,$status,$zakup,$rozn,$ed_izmer,$lot,$desc,$img_fold,$fold_img,$img);
					echo '</td>';
					$j++;
						if($j == ($m+1)) echo '</tr>';
					}
  ...
  <tbody>';
		$i = 0;
			...
			$lot = mysql_result($result,$i,"lot");
				...
				if(((!$rozn)||($rozn >= $zakup))||($rozn&&($rozn < $zakup)&&($status < 3))){
				echo ItemList($class,$zakup,$rozn,$ed_izmer,$lot,$id,$name,$is_from,$urozhay,$status);
				}
			$i++;
			}
		echo '
