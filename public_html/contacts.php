...
require ('supple/auth_db.php');
	...
	if($_SESSION['city']){
	$str_sql_query = 'SELECT contacts FROM cities WHERE name = "'.$_SESSION['city'].'"';
	$result = mysql_query($str_sql_query) or die(mysql_error());
	$number = mysql_num_rows($result);
		if($number){
		$i = 0;
		...
		$contacts = str_replace('<a href="http://www.','<a href="/go.php?be=1&li=',$contacts);
		$contacts = str_replace('<a href="http://','<a href="/go.php?li=',$contacts);
...
<title>Контактная информация проекта Ирий Сад: <?php echo $_SESSION['city']; ?></title>
</head>
...
?>
    ...
    <td width="200"><img src="images/head2_02.jpg" alt="Кооператив сыроедов. Контакты" width="200" height="150"></td>
...
?>
        ...
        <td width="88%" valign="top" style="border-left:1px solid #FFFFFF;"><table width="100%"  border="0" cellspacing="0" cellpadding="10" bgcolor="#FFFFFF" style="border-top:1px solid #758DB3; border-left:1px solid #758DB3; border-right:1px solid #758DB3; border-bottom:1px solid #758DB3;">
          <tr>
            <td><h2>Контактная информация: <?php echo $_SESSION['city']; ?></h2></td>
          </tr>
          <tr>
            <td class="text"><p align="justify"><?php echo $contacts; ?></p></td>
          </tr>
        </table></td>
        </tr>
    </table></td>
